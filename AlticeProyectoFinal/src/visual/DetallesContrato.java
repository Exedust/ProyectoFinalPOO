...
public class DetallesContrato extends JDialog {
...

				{
					...
					btnRealizarPago.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							...
							RegistrarPagoDirecto pago = new RegistrarPagoDirecto(
									Altice.getInstance().buscarPagoByCodigo(codigoPago));
							pago.setModal(true);
	...
	private void cancelarPago() {
		...
		if (pago == null) {
			JOptionPane.showMessageDialog(this, "No se encontró el pago seleccionado.", "Error",
					JOptionPane.ERROR_MESSAGE);
			return;
		}

		if (!pago.isPendiente() && pago.isActivo()) {
			JOptionPane.showMessageDialog(this, "No se puede cancelar un pago que ya ha sido realizado.",
					"Acción no permitida", JOptionPane.WARNING_MESSAGE);
			return;
		}

		if (!pago.isActivo()) {
			JOptionPane.showMessageDialog(this, "Este pago ya está cancelado.", "Acción no permitida",
		...
		if (rolLogueado == Rol.CLIENTE || rolLogueado == Rol.TECNICO) {
			JOptionPane.showMessageDialog(this, "Solo los comerciales y administradores pueden cancelar pagos.",
		...
		if (pago.getCliente() != null && pago.getCliente().getCedula() != null
			...
			JOptionPane.showMessageDialog(this, "No puedes cancelar un pago que está registrado a tu propio nombre.",
					"Acceso denegado", JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		int opcion = JOptionPane.showConfirmDialog(this,
				"¿Desea cancelar este pago pendiente?\n\n" + "Código: " + pago.getCodigo() + "\n" + "Monto: RD$ "
						...
						+ (pago.getCliente() != null ? pago.getCliente().getNombre() : "N/A"),
				"Confirmar Cancelación de Pago", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
		...
		if (Altice.getInstance().cancelarPago(codigoPago)) {
			JOptionPane.showMessageDialog(this, "Pago cancelado correctamente", "Éxito",
					JOptionPane.INFORMATION_MESSAGE);

			loadHistorialPagos(comboFiltrarPagos.getSelectedItem().toString());
		} else {
			JOptionPane.showMessageDialog(this, "No se pudo cancelar el pago", "Error", JOptionPane.ERROR_MESSAGE);
		}
	}

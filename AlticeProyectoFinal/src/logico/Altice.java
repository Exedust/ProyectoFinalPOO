...
public class Altice implements Serializable {
	...
	public void cargarDatosDePrueba() {
	    ...
	    // ====================== 1. SERVICIOS ======================
	    Servicio servInternet = null;
	    Servicio servCable = null;
	    Servicio servMovil = null;

	    if (!existeServicio(TipoServicio.INTERNET)) {
	        servInternet = new Servicio(TipoServicio.INTERNET, "Internet Fibra Óptica");
	        registrarServicio(servInternet);
	        System.out.println("✓ Servicio Internet creado");
	    } else {
	        servInternet = getMisServicios().stream()
	                .filter(s -> s.getTipo() == TipoServicio.INTERNET)
	                .findFirst().orElse(null);
	    }

	    if (!existeServicio(TipoServicio.CABLE)) {
	        servCable = new Servicio(TipoServicio.CABLE, "Cable HD y Premium");
	        registrarServicio(servCable);
	        System.out.println("✓ Servicio Cable creado");
	    } else {
	        servCable = getMisServicios().stream()
	                .filter(s -> s.getTipo() == TipoServicio.CABLE)
	                .findFirst().orElse(null);
	    }

	    if (!existeServicio(TipoServicio.MOVIL)) {
	        servMovil = new Servicio( TipoServicio.MOVIL, "Telefonía Móvil 5G");
	        registrarServicio(servMovil);
	        System.out.println("✓ Servicio Móvil creado");
	    } else {
	        servMovil = getMisServicios().stream()
	                .filter(s -> s.getTipo() == TipoServicio.MOVIL)
	                .findFirst().orElse(null);
	    }

	    // ====================== 2. PLANES ======================
	    if (buscarPlanByNombre("Plan Básico") == null && servInternet != null && servCable != null) {
	        Plan planBasico = new Plan("P-00001", "Plan Básico", "Internet 100 Mbps + Cable Básico");
	        planBasico.agregar(servInternet, 899.99f);
	        planBasico.agregar(servCable, 600f);
	        registrarPlan(planBasico);
	        System.out.println("✓ Plan Básico creado");
	    }

	    if (buscarPlanByNombre("Plan Premium") == null && servInternet != null && servCable != null && servMovil != null) {
	        Plan planPremium = new Plan("P-00002", "Plan Premium", "Internet 500 Mbps + Cable Premium + Móvil");
	        planPremium.agregar(servInternet, 1599.99f);
	        planPremium.agregar(servCable, 800f);
	        planPremium.agregar(servMovil, 600f);
	        registrarPlan(planPremium);
	        System.out.println("✓ Plan Premium creado");
	    }

	    if (buscarPlanByNombre("Plan Empresa") == null && servInternet != null) {
	        Plan planEmpresa = new Plan("P-00003", "Plan Empresa", "Internet 1 Gbps corporativo");
	        planEmpresa.agregar(servInternet, 4500f);
	        registrarPlan(planEmpresa);
	        System.out.println("✓ Plan Empresa creado");
	    }

	    // ====================== 3. EMPLEADOS ======================
	    if (buscarEmpleadoById("E-00001") == null) {
	        Usuario u1 = new Usuario("E-00001", "tecnico@altice.com", "1234", Rol.TECNICO);
	        Empleado tec = new Empleado("Victor Morales", "402-1234567-8", "tecnico@altice.com", 
	                                   "809-555-1111", "Santo Domingo", u1, 0f, 45000f);
	        registrarEmpleado(tec);
	        System.out.println("✓ Técnico Victor creado");
	    }

	    if (buscarEmpleadoById("E-00002") == null) {
	        Usuario u2 = new Usuario("E-00002", "comercial@altice.com", "1234", Rol.COMERCIAL);
	        Empleado com = new Empleado("Claudio Ramirez", "402-9876543-2", "comercial@altice.com", 
	                                   "809-555-2222", "Santo Domingo", u2, 15f, 38000f);
	        registrarEmpleado(com);
	        System.out.println("✓ Comercial Claudio creado");
	    }

	    // ====================== 4. CLIENTES ======================
	    if (buscarClienteByCedula("402-1111111-1") == null) {
	        Usuario uc1 = new Usuario("CL-00001", "juan.perez@gmail.com", "1234", Rol.CLIENTE);
	        Cliente c1 = new Cliente("Juan Pérez", "402-1111111-1", "juan.perez@gmail.com", 
	                                "809-333-4444", "Santo Domingo", uc1);
	        registrarCliente(c1);
	        System.out.println("✓ Cliente Juan Pérez creado");
	    }

	    if (buscarClienteByCedula("402-2222222-2") == null) {
	        Usuario uc2 = new Usuario("CL-00002", "empresa@techcorp.com", "1234", Rol.CLIENTE);
	        uc2.setEmpresa(true);
	        Cliente c2 = new Cliente("TechCorp SRL", "402-2222222-2", "empresa@techcorp.com", 
	                                "809-777-8888", "Santo Domingo", uc2);
	        registrarCliente(c2);
	        System.out.println("✓ Cliente Empresa TechCorp creado");
	    }

	    // ====================== 5. CONTRATOS ======================
	    Cliente cliente1 = buscarClienteByCedula("402-1111111-1");
	    Empleado tecnico = buscarEmpleadoById("E-00001");
	    Plan planBasico = buscarPlanByNombre("Plan Básico");

	    if (cliente1 != null && tecnico != null && planBasico != null) {
	        if (buscarContratoByCodigo("C-00001") == null) {
	            Contrato cont1 = new Contrato("C-00001", tecnico, cliente1, planBasico);
	            registrarContrato(cont1);
	            System.out.println("✓ Contrato C-00001 creado para Juan Pérez");
	        }
	    }

	    Cliente cliente2 = buscarClienteByCedula("402-2222222-2");
	    Plan planPremium = buscarPlanByNombre("Plan Premium");

	    if (cliente2 != null && tecnico != null && planPremium != null) {
	        if (buscarContratoByCodigo("C-00002") == null) {
	            Contrato cont2 = new Contrato("C-00002", tecnico, cliente2, planPremium);
	            registrarContrato(cont2);
	            System.out.println("✓ Contrato C-00002 creado para TechCorp");
	        }
	    }
